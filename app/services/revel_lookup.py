...
def _segment_for_position(revel_root: Path, pos: int, position_field: str) -> Path | None:
    for path in revel_root.glob("*.csv"):
        ...
        try:
            ...
            if len(parts) != 3:
                continue
            # REVEL segment archives encode the genomic chunk using the final
            # two numeric suffixes. Those ranges work as a coarse file locator
            # for both hg19 and GRCh38 rows inside the segment CSV.
            start = int(parts[1])
            end = int(parts[2])
        except ValueError:
            continue
